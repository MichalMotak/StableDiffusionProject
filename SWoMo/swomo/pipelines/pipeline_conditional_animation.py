        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if first_frame_latents is not None:
                    ...
                    if class_labels is not None:
                        noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings, first_frame_latents=first_frame_latents_input, frame_stride=frame_stride, class_labels=class_labels).sample.to(dtype=latents_dtype)
                    else:
                    ...
                    if class_labels is not None:
                        noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings, class_labels=class_labels).sample.to(dtype=latents_dtype)
                    else:
