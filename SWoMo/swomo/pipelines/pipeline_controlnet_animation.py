...
from ..models.videoldm_controlnet import ControlNetModel
...
class ControlNetAnimationPipeline(DiffusionPipeline):
    ...
    def check_inputs(
        ...
        if cond_video_frames_paths is not None:
            if not (
                isinstance(cond_video_frames_paths, list) and
                all(isinstance(inner, list) for inner in cond_video_frames_paths)
                or isinstance(cond_video_frames_paths, list)
            ):
                raise ValueError("`cond_video_frames_paths` must be a list or list of lists of strings.")
    ...
    def __call__(
        ...
        # controlnet
        cond_video_frames_paths: Optional[List[List[str]]] = None,
        cond_video_frames: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        if cond_video_frames_paths is not None and cond_video_frames is not None:
            raise ValueError("Only one of `first_frame_paths` and `first_frames` can be passed.")
        elif cond_video_frames_paths is None and cond_video_frames is None:
            raise ValueError("Need either `cond_video_frames_paths` or `cond_video_frames` to perform controlnet video2video synthesisation.")
        ...
        if not isinstance(control_guidance_start, list) and isinstance(control_guidance_end, list):
            ...
            control_guidance_start, control_guidance_end = [control_guidance_start], [
       ...
       # 0. Default height and width to unet
        ...
        if isinstance(prompt, list):
            ...
            cond_video_input = cond_video_frames_paths if cond_video_frames_paths is not None else cond_video_frames
            assert len(prompt) == len(cond_video_input), "prompt and cond_video_frames_paths should have the same length"
        ...
        # 4.1 Prepare first frame image and encode to latents
        ...
        # 4.2 Prepare cond video frames and encode to latents
        if cond_video_frames_paths is not None:
            cond_video_frames_paths = cond_video_frames_paths if all(isinstance(inner, list) for inner in cond_video_frames_paths) else [cond_video_frames_paths] * batch_size
            
            cond_video_frames_batch = []
            for cond_video_frame_paths in cond_video_frames_paths:
                print(cond_video_frame_paths)
                cond_video_frames = []
                for cond_video_frame_path in cond_video_frame_paths:
                    video_frame = Image.open(cond_video_frame_path).convert('RGB')
                    video_frame = img_transform(video_frame)
                    cond_video_frames.append(video_frame.unsqueeze(0))
                cond_video_frames = torch.cat(cond_video_frames, dim=0)
                cond_video_frames_batch.append(cond_video_frames.unsqueeze(0))

            cond_video_frames_batch = torch.cat(cond_video_frames_batch, dim=0)
        cond_video_frames_batch = cond_video_frames_batch.to(device, dtype=self.vae.dtype)
        cond_video_frames = rearrange(cond_video_frames_batch, "b f c h w -> b c f h w")
        ...
        if class_labels is not None:
            class_labels = torch.cat([class_labels] * 2).to(device)

        if do_classifier_free_guidance is None:
            cond_video_frames = cond_video_frames
        elif do_classifier_free_guidance == "text":
            cond_video_frames = torch.cat([cond_video_frames] * 2)
        elif do_classifier_free_guidance == "both":
            cond_video_frames = torch.cat([cond_video_frames] * 3)
            ...
            for i, t in enumerate(timesteps):
               ...
               # controlnet(s) inference
                if guess_mode and do_classifier_free_guidance:
                    ...
                    raise NotImplementedError("Guess mode is not implemented yet.")
                ...
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    ...
                    controlnet_cond=cond_video_frames,
                    conditioning_scale=cond_scale,  
                    class_labels=class_labels,
                    guess_mode=guess_mode,
                    first_frame_latents=first_frame_latents_input if first_frame_latents is not None else None,
                ...
                down_block_res_samples = [sample.to(dtype=latents_dtype) for sample in down_block_res_samples]
                mid_block_res_sample = mid_block_res_sample.to(dtype=latents_dtype)
                
                if first_frame_latents is not None:
                    # predict the noise residual
                    if class_labels is not None:
                        noise_pred = self.unet(latent_model_input, t, down_block_additional_residuals=down_block_res_samples, mid_block_additional_residual=mid_block_res_sample, encoder_hidden_states=text_embeddings, first_frame_latents=first_frame_latents_input, frame_stride=frame_stride, class_labels=class_labels).sample.to(dtype=latents_dtype)
                    else:
                        noise_pred = self.unet(latent_model_input, t, down_block_additional_residuals=down_block_res_samples, mid_block_additional_residual=mid_block_res_sample, encoder_hidden_states=text_embeddings, first_frame_latents=first_frame_latents_input, frame_stride=frame_stride).sample.to(dtype=latents_dtype)
                else:
                    if class_labels is not None:
                        noise_pred = self.unet(latent_model_input, t, down_block_additional_residuals=down_block_res_samples, mid_block_additional_residual=mid_block_res_sample, encoder_hidden_states=text_embeddings, class_labels=class_labels).sample.to(dtype=latents_dtype)
                    else:
                        noise_pred = self.unet(latent_model_input, t, down_block_additional_residuals=down_block_res_samples, mid_block_additional_residual=mid_block_res_sample, encoder_hidden_states=text_embeddings).sample.to(dtype=latents_dtype)
