...
def save_videos_grid_from_paths(video_paths: list, path: str, rescale=False, n_rows=8, fps=8, wandb=False, global_step=0, resize=None, format="gif"):
    videos = []
    outputs = []
    for vid in video_paths:
        x = [T.ToTensor()(Image.open(img)) for img in vid]
        if resize is not None:
            x = [T.Resize(resize)(img) for img in x]
        videos.append(torch.stack(x, dim=0))
...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=8, fps=8, wandb=False, global_step=0, format="gif"):
