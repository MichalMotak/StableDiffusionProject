...
from my_datasets import DatasetRetinaVideoBatches, DatasetRetinaGIGVideo, DatasetRetinaVideoBatchesSim

from surgora_my_utils import get_cmp_flow, get_dav2_flows, grayscale_to_color_mask, overlay_segmentation, get_optical_flows_main_optimized, sample_inputs, get_optical_flows, get_optical_flows_backwards
...
def main():
    ...
    test_dataset = DatasetRetinaVideoBatchesSim(df_path = args.test_dataset,
                                                limit = 0)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                # print("pixel_values", pixel_values.shape, torch.min(pixel_values), torch.max(pixel_values))
                ...
                # print("depths_ds ", depths_ds.shape, depths.shape)
                # for f_ind in range(depths_ds.shape[0]):
                #     if torch.max(depths_ds[f_ind]) == 0:
                #         # print("change depths")
                #         depths[f_ind] = depths_ds[f_ind]

                # print("depths after ", depths.shape)


                # flows_ds = batch["flows"]

                # if args.flow_backwards:
                #     flows = get_optical_flows_backwards(unimatch, pixel_values) 

                flows = get_optical_flows_main_optimized(unimatch, pixel_values, args.flow_backwards, args.flow_close)



                # masks_sam = get_sam_flows(predictor, pixel_values)
                # print("flows ")
                # print(flows_ds.shape, flows.shape)

                # for f_ind in range(flows_ds.shape[0]):
                #     if torch.max(flows_ds[f_ind]) == 0:
                #         # print("change flows")
                #         flows[f_ind] = flows_ds[f_ind]
                # print("flows after ", flows.shape)
                ...
                if (
                    ...
                    with torch.autocast(
                        ...
                        for val_img_idx in range(args.num_validation_images):
                            ...
                            val_flows = get_optical_flows_main_optimized(unimatch, val_pixel_values, args.flow_backwards, args.flow_close)
