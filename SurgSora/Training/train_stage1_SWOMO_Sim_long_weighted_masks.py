...
from torch.utils.data import RandomSampler,  DataLoader, ConcatDataset, Sampler
import transformers
...
# Add the parent directory to Python's searchable path
sys.path.append(parent_dir)
from swomo.data.dataset import SurgicalDataset_my
from omegaconf import OmegaConf


from my_datasets import DatasetRetinaVideoBatches, DatasetRetinaGIGVideo, DatasetRetinaVideoBatchesSim, SurgicalDataset_Sim

from surgora_my_utils import get_cmp_flow, get_dav2_flows, grayscale_to_color_mask, overlay_segmentation, sample_inputs, get_optical_flows_main_optimized
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Perc of steps for the warmup in the lr scheduler.",
    )
    parser.add_argument(
...
def main():
    ...
    # setup_cuda(use_memory_fraction=0.5, num_threads=4, visible_devices="1,2", multiGPU=True)
    ...
    # train_dataset = DatasetRetinaVideoBatchesSim(df_path = args.train_dataset1,
    #                                             limit = 20)

    config = OmegaConf.load("/net/storage/pr3/plgrid/plggmultigeomed/mmotak/SurgSora/SWoMo/configs/training/ds_train_test.yaml")

    config.train_data.split_mode = args.train_dataset_split
    config.train_data.sample_size = args.width
    print(config.train_data)

    # config.train_graph_encoder = False
    train_dataset = SurgicalDataset_Sim(**config.train_data, train_graph_encoder = True)
    ...
    config_test = OmegaConf.load("/net/storage/pr3/plgrid/plggmultigeomed/mmotak/SurgSora/SWoMo/configs/training/ds_val.yaml")

    config_test.train_data.split_mode = "val"
    config_test.train_data.sample_size = args.width

    print(config_test.train_data)
    test_dataset = SurgicalDataset_Sim(**config_test.train_data, train_graph_encoder = True)
    ...
    lr_warmup_steps = args.max_train_steps * args.lr_warmup_steps_per
    ...
    controlnet.mask_loss_weight = args.controlnet_masks_weight
    print("controlnet.mask_loss_weight ", controlnet.mask_loss_weight)
    ...
    if accelerator.is_main_process and not os.path.exists(csv_log_path):
        with open(csv_log_path, mode="w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["epoch", "global_step", "mean_epoch_loss", "mean_epoch_loss_w", "lr", "epoch_duration", "cumulative_time"])
    ...
    # 1. Print a summary ONLY from the main process (so it doesn't print twice)
    if accelerator.is_main_process:
        print("\n" + "="*50)
        print("🚀 HARDWARE ALLOCATION CHECK 🚀")
        print("="*50)
        
        # Check GPU/Process count
        print(f"Total GPUs/Processes active: {accelerator.num_processes}")
        print(f"Distributed Training Type:   {accelerator.distributed_type}")
        print(f"Mixed Precision Mode:        {accelerator.mixed_precision}")
        
        # Check what PyTorch natively sees
        print(f"PyTorch visible GPUs:        {torch.cuda.device_count()}")
        
        # Check CPU Workers (assuming your dataloader is named train_dataloader)
        if 'train_dataloader' in locals():
            print(f"CPU Workers (num_workers):   {train_dataloader.num_workers}")
        else:
            print("CPU Workers: (Dataloader not defined yet in this scope)")
            
        print("="*50 + "\n")

    # 2. Let EVERY process announce itself to prove they are both running
    print(f"✅ Hello from Process {accelerator.process_index}! I am using {accelerator.device}")


    if args.mask_loss_weight > 1.0:
        print("Training in mask_weight mode ", args.mask_loss_weight)

    if args.tool_loss_weight > 1.0:
        print("Training in tool_loss_weight mode ", args.tool_loss_weight)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                sim_cond_pixel_values = batch["cond_video_values"].to(weight_dtype).to(
                    accelerator.device, non_blocking=True
                )
                ...
                train_noise_aug = args.train_noise_aug
                ...
                added_time_ids = _get_add_time_ids(
                    args.fps, # frame per second
                    args.motion_buket_ids, # motion buket ids: the motion bucket id to use for the generated video. This can be used to control the motion of the generated video. Increasing the motion bucket id increases the motion of the generated video.
                ...
                # depths_ds = batch["depths"]

                depths,_ = get_dav2_flows(depth_anything, sim_cond_pixel_values) 
                depths = depths.to(torch.float16).to(accelerator.device)



                flows = get_optical_flows_main_optimized(unimatch, sim_cond_pixel_values, args.flow_backwards, args.flow_close)
                ...
                if args.width == 256:
                    ...
                    dsi_mask_spatial = F.interpolate(my_gt_mask, size=(128, 128), mode='nearest')         
                ...
                # masks = F.pad(dsi_mask_spatial, (0, 0, 0, 0, 0, args.width - args.num_frames)).to(torch.float16).to(accelerator.device) # Pad 14 to 256
                target_channels = args.width
                current_channels = dsi_mask_spatial.shape[1]
                padding_needed = target_channels - current_channels
                masks = F.pad(dsi_mask_spatial, (0, 0, 0, 0, 0, padding_needed)).to(torch.float16).to(accelerator.device)
                # print("masks pad ", masks.shape )
                ###############################################################
            ...
            #####################3
                ...
                if args.mask_loss_weight > 1.0:
                    ...
                    dilated_weight_map = 1.0 + (dilated_latent_masks >= 1).float() * args.mask_loss_weight  # Tool = 10x weight

                if args.tool_loss_weight > 1.0:
                    dilated_latent_masks = dilated_latent_masks.view(B, F_frames, 1, H_latent, W_latent)
                    dilated_weight_map = 1.0 + (dilated_latent_masks >= 3).float() * args.tool_loss_weight  # Tool = 10x weight


                # print("loss ", loss.shape)

                loss_w = dilated_weight_map * loss
                # print("loss_w ", loss_w.shape)

                loss_w = torch.mean(loss_w.reshape(target.shape[0], -1), dim = 1)
                # print("loss_w mean", loss_w.shape, loss_w)
                loss_w_final = loss_w.mean()
                # print("loss_w_final ", loss_w_final.shape, loss_w_final)
            #####################3



                epoch_losses.append(loss_w_final.detach().item())


                # Gather the losses across all processes for logging (if we use distributed training).
                avg_loss = accelerator.gather(
                    loss_w_final.repeat(args.per_gpu_batch_size)).mean()
                train_loss += avg_loss.item() / args.gradient_accumulation_steps

                # Backpropagate
                accelerator.backward(loss_w_final)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            epoch_losses_w.append(loss_w_final.detach().item())
        ...
        if accelerator.is_main_process:
            with open(csv_log_path, mode="a", newline="") as f:
                ...
                mean_epoch_loss_w = np.mean(epoch_losses_w) if len(epoch_losses_w) > 0 else float("nan")
            ...
            logger.info(f"[LOG] Epoch {epoch}, Step {global_step}: "
                    f"mean_loss={mean_epoch_loss:.6f}, mean_loss_w={mean_epoch_loss_w:.6f}, global_mean_loss={global_min_loss:.6f}, lr={current_lr:.2e}, "
                ...
                if (
                    ...
                    with torch.autocast(
                        ...
                        for val_img_idx in range(args.num_validation_images):
                            ...
                            sim_cond_pixel_values = val_batch["cond_video_values"].to(weight_dtype).to(
                                accelerator.device, non_blocking=True
                            )
                            
                            # print("val_pixel_values ", val_pixel_values.shape)
                            val_depths,depth_img = get_dav2_flows(depth_anything, sim_cond_pixel_values)
                            val_depths = val_depths.to(torch.float16).to(accelerator.device)


                            val_flows = get_optical_flows_main_optimized(unimatch, sim_cond_pixel_values, args.flow_backwards, args.flow_close)
                            val_flows = val_flows.to(torch.float16).to(accelerator.device)


                            # depths_ds_val = batch["depths"]

                            # print("depths_ds ", depths_ds_val.shape, val_depths.shape)
                            # for f_ind in range(depths_ds_val.shape[0]):
                            #     if torch.max(depths_ds_val[f_ind]) == 0:
                            #         # print("change depths")
                            #         val_depths[f_ind] = depths_ds_val[f_ind]

                            # print("depths after ", val_depths.shape)


                            # flows_ds_val = batch["flows"]
                            # masks_sam = get_sam_flows(predictor, pixel_values)
                            # print("flows ")
                            # print(flows_ds_val.shape, val_flows.shape)

                            # for f_ind in range(flows_ds_val.shape[0]):
                            #     if torch.max(flows_ds_val[f_ind]) == 0:
                            #         # print("change flows")
                            #         val_flows[f_ind] = flows_ds_val[f_ind]
                            # print("flows after ", val_flows.shape)
                            ...
                            # # 2. If the model expects 256 channels (SAM feature size), 
                            # # you can pad or project. A common trick is to repeat classes:
                            # val_masks = F.pad(dsi_mask_spatial, (0, 0, 0, 0, 0, 256 - args.num_frames)).to(torch.float16).to(accelerator.device) # Pad 14 to 256
                            ...
                            if args.width == 256:
                                ...
                                dsi_mask_spatial = F.interpolate(val_masks_org, size=(128, 128), mode='nearest')         
                            ...
                            current_channels = dsi_mask_spatial.shape[1]
                            padding_needed = target_channels - current_channels
                            val_masks = F.pad(dsi_mask_spatial, (0, 0, 0, 0, 0, padding_needed)).to(torch.float16).to(accelerator.device)
                            # print("masks pad ", masks.shape )
                            ...
                            video_frames = pipeline(
                                ...
                                decode_chunk_size=8,
                                motion_bucket_id=args.motion_buket_ids,
                                fps=args.fps,
                                noise_aug_strength=args.train_noise_aug,
                                # generator=generator,
                            ).frames[0]
                            ...
                            for i in range(val_flows.shape[1]):
                                ...
                                viz_flows.append(flow_to_image(temp_flow.clone()))
