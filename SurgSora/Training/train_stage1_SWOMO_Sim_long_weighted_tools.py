...
# def get_optical_flows_backwards(unimatch, video_frame):
#     '''
#         video_frame: [b, t, c, w, h] -> MUST be float32 in range [0, 1]
#     '''
    
#     # REMOVED: video_frame = video_frame * 255 
    
#     flows = []
#     for i in range(video_frame.shape[1] - 1):
        
#         # Backward flow: Target Frame (i+1) back to Anchor Frame (0)
#         image1, image2 = video_frame[:, i + 1], video_frame[:, 0]

#         image1_r, image2_r, inference_size, ori_size, transpose_img = preprocess_size(image1, image2)
        
#         # CRITICAL: Disable gradients for clean, stable inference
#         with torch.no_grad():
...
#         flow_r = results_dict_r['flow_preds'][-1]  
#         flow = postprocess_size(flow_r, inference_size, ori_size, transpose_img)
#         flows.append(flow.unsqueeze(1)) 
    
#     # CRITICAL: Must be float32 for OpenCV cv2.remap compatibility
#     flows = torch.cat(flows, dim=1).to(torch.float32)  
#     return flows
...
def main():
    ...
    config.train_data.split_mode = "train"
    ...
    controlnet.mask_weight = args.controlnet_masks_weight
