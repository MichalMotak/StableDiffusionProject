...
class Unet(nn.Module):
    def __init__(
        ...
        # time embeddings
        multiplier = num_classes // dim
        time_dim = dim * multiplier
        ...
        # class embeddings
        classes_dim = dim * multiplier
      
        # self.null_classes_emb = nn.Parameter(torch.full((1,num_classes), -1, dtype = torch.float32).squeeze())
        self.null_classes_emb = nn.Parameter(torch.randn(num_classes))
        self.classes_mlp = nn.Identity()
    ...
    def forward(
        ...
        if cond_drop_prob > 0:
            ...
            if non_labeled_image is not None:
                x = torch.where(
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, *, classes, non_labeled_image_start = None, noise = None):
        ...
        if non_labeled_image_start is not None:
            non_labeled_image_start = normalize_to_neg_one_to_one(non_labeled_image_start) 
            non_labeled_image = self.q_sample(x_start = non_labeled_image_start, t = t, noise = noise)
            model_out = self.model(x, t, classes, non_labeled_image=non_labeled_image)
        else:
