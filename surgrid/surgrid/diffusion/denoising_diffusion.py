...
from surgrid.diffusion.attend import Attend
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        loss = self.model(data[0], classes = data[1])
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            sample = self.ds.val_graph_emb
                            sample = sample.to(dtype=torch.float32, device=device)
                            batches = num_to_groups(self.num_samples, self.batch_size)
                            batches = list(torch.split(sample, batches))
                            all_images_list = list(map(lambda n: self.ema.ema_model.sample(classes = n, cond_scale = 2.5), batches))
