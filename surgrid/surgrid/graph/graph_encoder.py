...
class GCNGraphEncoder(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, z_dim: int, dropout: float = 0.5, global_pooling: bool = False):
        """
        ...
        self.conv4 = GCNConv(hidden_dim, z_dim)
...
class GVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.z_dim = z_dim
        if node_pooling: self.encoder = GCNGraphEncoder(input_dim, hidden_dim, z_dim, dropout, global_pooling=True)
        else: self.encoder = GCNGraphEncoder(input_dim, hidden_dim, z_dim, dropout)
