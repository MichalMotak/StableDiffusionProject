...
def sample_inputs_rescale_flows(unimatch, pixel_values, flows):
    ...
    return sparse_optical_flow_384



def sample_inputs(unimatch, pixel_values):

    flows = get_optical_flows_1t_backwards(unimatch, pixel_values) 
...
@torch.no_grad()
def get_optical_flows_1t_backwards(unimatch, video_frame):
    '''
        video_frame: [b, t, c, w, h]
    '''
...
def main():
    ...
    setup_cuda(use_memory_fraction=0.9, num_threads=16, visible_devices="0", multiGPU=False)
    ...
    train_dataset2 = DatasetRetinaGIGVideo(main_path = "/home/MichalMo/projects/SurgSora/Training/GIG_dataset/dataset_keyth_004_mth_03", limit = 501, mask_mode = "sim_to_swomo")


    combined_dataset = ConcatDataset([train_dataset1, train_dataset2])

    # 2. Create the custom sampler
    balanced_sampler = BalancedBatchSampler(len(train_dataset1), len(train_dataset2), batch_size=args.per_gpu_batch_size)
    ...
    test_dataset = DatasetRetinaVideoBatches(df_path = "/home/MichalMo/projects/SurgSora/SWoMo/datasets/Cataract-1K/dataset_info/training_batch_paths/paths_eval_batches_21_frames_classes_0,1,2.csv")
    ...
    logger.info(f"  Num examples = {len(combined_dataset)}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                flows_ds_rescaled = sample_inputs_rescale_flows(unimatch, pixel_values, flows_ds)
                # print("flows_ds_rescaled ", flows_ds_rescaled.shape)

                for f_ind in range(flows_ds_rescaled.shape[0]):
                    if torch.max(flows_ds_rescaled[f_ind]) == 0:
                        # print("change flows")
                        controlnet_flow[f_ind] = flows_ds_rescaled[f_ind]
                # print("flows after ", controlnet_flow.shape)
                ...
                controlnet_depth = controlnet_depth.to(torch.float16).to(accelerator.device)

                for f_ind in range(depths_ds.shape[0]):
                    if torch.max(depths_ds[f_ind]) == 0:
                        # print("change depths")
                        controlnet_depth[f_ind] = depths_ds[f_ind]
                ...
                if (
                    ...
                    with torch.autocast(
                        ...
                        for val_img_idx in range(args.num_validation_images):
                            ...
                            # print(val_controlnet_flow.shape)

                            # flows_ds = batch["flows"]

                            # val_controlnet_flow = sample_inputs_rescale_flows(unimatch, pixel_values, flows_ds)
                            # print("val_controlnet_flow rescaled ", val_controlnet_flow.shape)

                            # for f_ind in range(flows_ds_rescaled.shape[0]):
                            #     if torch.max(flows_ds_rescaled[f_ind]) == 0:
                            #         # print("change flows")
                            #         val_controlnet_flow[f_ind] = flows_ds_rescaled[f_ind]
                            # print("flows after ", val_controlnet_flow.shape)
                            ...
                            # print("val_controlnet_flow ", val_controlnet_flow.shape)

                            # val_controlnet_depth = batch["depths"]
                            ...
                            # print("val_masks_org ", val_masks_org.shape)
                            ...
                            # print("val_controlnet_masks shape ", val_controlnet_masks.shape, val_controlnet_masks.dtype, val_controlnet_masks.device)

                            # print("val_controlnet_flow shape ", val_controlnet_flow.shape, val_controlnet_flow.dtype, val_controlnet_flow.device)
                            # print("val_controlnet_depth shape ", val_controlnet_depth.shape, val_controlnet_depth.dtype, val_controlnet_depth.device)
